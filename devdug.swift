...
struct CleanupAction {
    ...
    let explanation: String? // what this does, why it's safe
...
func getCleanupActions(for projectType: String, at projectPath: String) -> [CleanupAction] {
    ...
    case "python-pip", "python-poetry", "python-setuptools":
        ...
        actions.append(CleanupAction(type: .pathDelete, description: "dist/", explanation: "Built distributions (wheels, eggs). Regenerate with build tools.", estimatedSize: nil))
        actions.append(CleanupAction(type: .pathDelete, description: "build/", explanation: "Intermediate build files. Safe to delete.", estimatedSize: nil))
        actions.append(CleanupAction(type: .pathDelete, description: "*.egg-info/", explanation: "Package metadata. Regenerated by setuptools on next install.", estimatedSize: nil))
        
    case "make":
        let makefilePath = "\(projectPath)/Makefile"
        if fm.fileExists(atPath: makefilePath) {
            actions.append(CleanupAction(type: .tentativeCommand, description: "make clean (if target exists)", explanation: "Makefile's clean target. Varies by project.", estimatedSize: nil))
        }
        actions.append(CleanupAction(type: .pathDelete, description: "build/", explanation: "Common build output directory.", estimatedSize: nil))
        
    case "cmake":
        actions.append(CleanupAction(type: .tentativeCommand, description: "make clean (if CMakeLists.txt)", explanation: nil, estimatedSize: nil))
        actions.append(CleanupAction(type: .pathDelete, description: "cmake-build-debug/", explanation: "CMake build artifacts. Regenerates on next build.", estimatedSize: nil))
        actions.append(CleanupAction(type: .pathDelete, description: "cmake-build-release/", explanation: "CMake build artifacts. Regenerates on next build.", estimatedSize: nil))
        
    case "intellij-idea":
        actions.append(CleanupAction(type: .pathDelete, description: ".idea/cache/", explanation: "IntelliJ IDEA code completion and inspection caches. Regenerated on next open.", estimatedSize: nil))
        actions.append(CleanupAction(type: .pathDelete, description: ".idea/caches/", explanation: "JetBrains IDE caches. Regenerated automatically.", estimatedSize: nil))
        actions.append(CleanupAction(type: .pathDelete, description: ".idea/shelf/", explanation: "Shelved changes (similar to git stash). Safe if you've committed important work.", estimatedSize: nil))
        
    case "eclipse-workspace":
        actions.append(CleanupAction(type: .pathDelete, description: ".recommenders/", explanation: "Eclipse code recommendations cache. Regenerated on next startup.", estimatedSize: nil))
        
    case "xcode":
        actions.append(CleanupAction(type: .pathDelete, description: "~/Library/Developer/Xcode/DerivedData/<ProjectName>-*/", explanation: "Xcode build artifacts and simulator data. Regenerates on next build.", estimatedSize: nil))
        
    case "go":
        if fm.fileExists(atPath: "\(projectPath)/vendor") {
            actions.append(CleanupAction(type: .tentativeCommand, description: "rm -rf vendor/", explanation: "Vendor dependencies. Regenerate with go mod download.", estimatedSize: nil))
        }
        
    case "git-repo":
        actions.append(CleanupAction(type: .safeCommand, description: "git gc --aggressive (optimize, don't delete)", explanation: "Optimizes git repository. Doesn't delete anything, just compresses.", estimatedSize: nil))
...
func printCleanupStrategy(_ projects: [ProjectInfo], verbose: Bool = false) {
    ...
    // Show top space-saving projects
